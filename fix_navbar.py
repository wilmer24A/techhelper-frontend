  ...
  return (
    <nav style={{background:"white",borderBottom:"1px solid #e5e7eb",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:"56px",position:"sticky",top:0,zIndex:100}}>
      ...
      <div style={{display:"flex",gap:"4px"}}>
        {links.map(link => <a key={link.href} href={link.href} style={{padding:"6px 14px",borderRadius:"6px",fontSize:"13px",fontWeight:500,color:pathname===link.href?"#111":"#6b7280",background:pathname===link.href?"#f3f4f6":"transparent",textDecoration:"none"}}>{link.label}</a>)}
      </div>
    </nav>
  )
}'''
